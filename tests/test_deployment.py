...
class TestSecureStorage:
    ...
    def setup_teardown(self):
        """Clean up API key files before/after tests"""
        # Store original keys if they exist
        original_uspto = get_uspto_api_key()
        original_mistral = get_mistral_api_key()

        yield

        # Restore original keys or delete test keys
        uspto_path = Path.home() / '.uspto_api_key'
        mistral_path = Path.home() / '.mistral_api_key'

        if original_uspto:
            store_uspto_api_key(original_uspto)
        elif uspto_path.exists():
            uspto_path.unlink()

        if original_mistral:
            store_mistral_api_key(original_mistral)
    ...
    def test_linux_file_permissions_secure(self):
        ...
        key_file = Path.home() / '.uspto_api_key'
