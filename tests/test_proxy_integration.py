...
def test_detect_pfw_proxy_explicit_port():
    ...
    # Could return None if PFW not running, or a base URL if it is running
...
def test_centralized_base_url_precedence(monkeypatch):
    """CENTRALIZED_PROXY_URL (full URL) wins over legacy CENTRALIZED_PROXY_PORT."""
    from ptab_mcp.proxy.centralized_integration import get_centralized_base_url

    monkeypatch.setenv('CENTRALIZED_PROXY_URL', 'http://pfw:8080/')
    monkeypatch.setenv('CENTRALIZED_PROXY_PORT', '9999')
    assert get_centralized_base_url() == 'http://pfw:8080'

    # Legacy port fallback when URL unset
    monkeypatch.delenv('CENTRALIZED_PROXY_URL')
    assert get_centralized_base_url() == 'http://localhost:9999'

    # HTTPS remote base is accepted
    monkeypatch.setenv('CENTRALIZED_PROXY_URL', 'https://pfw-proxy.example.com')
    assert get_centralized_base_url() == 'https://pfw-proxy.example.com'

    # Invalid scheme is rejected -> falls back to None (port also invalid here)
    monkeypatch.setenv('CENTRALIZED_PROXY_URL', 'ftp://bad')
    monkeypatch.setenv('CENTRALIZED_PROXY_PORT', 'none')
    assert get_centralized_base_url() is None


def test_centralized_base_url_none_sentinel(monkeypatch):
    """'none'/unset configuration disables centralized proxying."""
    from ptab_mcp.proxy.centralized_integration import get_centralized_base_url

    monkeypatch.delenv('CENTRALIZED_PROXY_URL', raising=False)
    monkeypatch.setenv('CENTRALIZED_PROXY_PORT', 'none')
    assert get_centralized_base_url() is None
