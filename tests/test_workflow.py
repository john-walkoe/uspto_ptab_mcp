...
class TestProxyIntegration:
    ...
    async def test_document_download_includes_proxy_info(self):
        ...
        if 'error' not in download_data:
            ...
            # Local mode must emit token-in-path persistent links (Lesson 43)
            if download_data['proxy_info']['mode'] == 'local':
                assert '/download/persistent/' in download_data['download_url']
        else:
