...
class FieldManager:
    ...
    def filter_response_custom(self, data: Dict[str, Any], custom_fields: List[str]) -> Dict[str, Any]:
        ...
        if "*" in custom_fields:
            logger.debug(f"Wildcard '*' detected in custom fields, returning complete data")
            return data
        ...
        if results_key and results_key in data and isinstance(data[results_key], list):
            ...
            for item in data[results_key]:
                ...
                expanded_fields = self._expand_wildcards(custom_fields, item)
            ...
            filtered_data["context_info"] = {
                "field_set": "custom",
                "fields_configured": len(custom_fields),
            ...
            logger.debug(f"Filtered response with custom fields: {len(filtered_results)} items with {len(expanded_fields)} fields each")
            return filtered_data

        else:
            # Single item or unexpected format
            expanded_fields = self._expand_wildcards(custom_fields, data)
    ...
    def filter_response(self, data: Dict[str, Any], field_set: str) -> Dict[str, Any]:
        ...
        # Handle wildcard "*" - return all data as-is
        if "*" in fields:
            logger.debug(f"Wildcard '*' detected, returning complete data")
            return data
        ...
        if results_key and results_key in data and isinstance(data[results_key], list):
            ...
            logger.debug(f"Filtered response: {len(filtered_results)} items with {len(expanded_fields)} fields each")
            return filtered_data
