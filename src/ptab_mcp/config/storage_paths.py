...
# ===== Runtime data directory (M-7) =====
#
# Stateful runtime artifacts (persistent-link SQLite cache + its Fernet key)
# live in a dedicated 0700 directory instead of the repo/image root, so a
# volume mount at ONE path (or PTAB_DATA_DIR pointing at one) keeps 7-day
# persistent links valid across container redeploys.


def get_data_dir() -> Path:
    """Runtime data directory: $PTAB_DATA_DIR or ~/.uspto_ptab_mcp/data (0700)."""
    override = os.getenv("PTAB_DATA_DIR", "").strip()
    data_dir = Path(override) if override else Path.home() / ".uspto_ptab_mcp" / "data"
...
def migrate_data_file(filename: str, legacy_dir: Path) -> Path:
    """Return the data-dir path for `filename`, moving a legacy copy if found.

    Migrates `legacy_dir/filename` (plus SQLite -wal/-shm sidecars) into the
    data dir on first use, so existing deployments keep their encryption key
    and outstanding persistent links.
    """
