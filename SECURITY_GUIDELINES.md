...
- **Admin-tool registration gating** — `ptab_manage_users` is registered only when `PTAB_ENABLE_USER_MANAGEMENT=true` (default off); absent from `tools/list` entirely otherwise. OAuth deployments must set this true or the admin tool disappears.
- **Fail-fast scope verification** — startup refuses to run if the admin scope gate fails to attach to the registered admin tool, preventing undefined authorization state.
- **Security-log content minimization** — masked emails/IPs, no raw tokens or query text reach the security log; flow metadata (tool, request ID, status, counts) only.
- **Refresh-token family revocation** — replay of a spent/rotated OAuth refresh token revokes every live token for that identity, blocking token-reuse attacks.
- **Shared cross-process USPTO rate limiter** — when all 4 USPTO MCPs (Citations, PFW, PTAB, FPD) run as containers on one host under one API key, set `USPTO_SHARED_RATE_LIMIT_DIR` to point all at a bind-mounted directory. A cross-process token bucket + concurrency-slot pool arbitrated via POSIX file locks (crash-safe) keep the combined request rate under USPTO's documented per-key limits. See README section below.
